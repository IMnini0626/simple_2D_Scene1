...
const char SPRITE_FILEPATH1[] = "pixel_person.png";
const char SPRITE_FILEPATH2[] = "shell.png";
...
GLuint g_texture_id1;
GLuint g_texture_id2;

// overall position
glm::vec3 g_object1_position = glm::vec3(0.0f, 0.0f, 0.0f);

// translation
float g_x_translate = 0.0f;

// Constants for rotating
const float RADIUS1 = 2.0f;      // radius of your circle
const float RADIUS2 = 1.5f;
const float ROT_SPEED1 = 1.0f;  // rotational speed
const float ROT_SPEED2 = 2.0f;
float       g_angle1 = 0.0f;     // current angle
float       g_angle2 = 0.0f;
float       g_x_coord1 = RADIUS1, // current x and y coordinates
            g_y_coord1 = 0.0f;
float       g_x_coord2 = RADIUS2,
            g_y_coord2 = 0.0f;

// Constants for self-rotation
const float ROT_ANGLE = glm::radians(180.0f);
float       g_angle3 = 0.0f;

// Constants for scaling
const float GROWTH_FACTOR = 1.0f;
const float SHRINK_FACTOR = -1.0f;
...
void initialise()
{
    ...
    g_texture_id1 = load_texture(SPRITE_FILEPATH1);
    g_texture_id2 = load_texture(SPRITE_FILEPATH2);
...
void update()
{
    ...
    g_angle1 += ROT_SPEED1 * delta_time;     // increment g_angle by ROT_SPEED

    // 2. Calculate x,y using trigonometry
    g_x_coord1 = RADIUS1 * glm::cos(g_angle1);
    g_y_coord1 = RADIUS1 * glm::sin(g_angle1);

    // 3. Reset the model matrix and apply transformation
    m_model_matrix1 = glm::mat4(1.0f);
    m_model_matrix1 = glm::translate(m_model_matrix1, glm::vec3(g_x_coord1, g_y_coord1, 0.0f));

    g_angle2 += ROT_SPEED2 * delta_time;

    g_x_coord2 = RADIUS2 * glm::cos(g_angle2);
    g_y_coord2 = RADIUS2 * glm::sin(g_angle2);

    m_model_matrix2 = m_model_matrix1;
    m_model_matrix2 = glm::translate(m_model_matrix2, glm::vec3(g_x_coord2, g_y_coord2, 0.0f));
    
    // Self rotation of object 2
    g_angle3 += ROT_ANGLE * delta_time;
    m_model_matrix2 = glm::rotate(m_model_matrix2, g_angle3, glm::vec3(0.0f, 0.0f, 1.0f));
    
    // Scaling of object 1
    g_time_counter += delta_time;
    
    if (g_time_counter >= MAX_TIME)
    {
        g_is_growing = !g_is_growing;
        g_time_counter = 0;
    }
    
    scale_vector += glm::vec3(g_is_growing ? (GROWTH_FACTOR * delta_time) : (SHRINK_FACTOR * delta_time),
                                 g_is_growing ? (GROWTH_FACTOR * delta_time) : (SHRINK_FACTOR * delta_time),
                                 0.0f);
    m_model_matrix1 = glm::scale(m_model_matrix1, scale_vector);
}
...
void render() {
    ...
    // Bind texture
    draw_object(m_model_matrix1, g_texture_id1);
    ...
    // Bind texture
    draw_object(m_model_matrix2, g_texture_id2);
